 ...
 * @ignore
 * @param {string}
 *            local Storage 혹은 cookie storage 에 저장할 value 값
 * @returns {*}
 * @private
 */
	this.setStateStore = function ()
	{
		try {
			...
			{
				var today = new Date();
				var expire = new Date(today.getTime() + 60 * 5 * 1000); // 5분간
																		// 유효
 ...
 */
	this.init_naver_id_login = function ()
	{
		...
		else if (this.button_type == CUSTOM_TYPE)
		{
 ...
 * @ignore
 * @param {string}
 *            oauth callback url 로 전달된 state 값
		...
		if (this.is_callback)
		{
			if (this.oauthParams.error==undefined)
			{
				if (this.oauthParams.access_token!=undefined)
				{
					...
					{
						/*
						 * state 가 맞지 않음 cookie 사용이 차단 된 경우 사용자가 명시적으로 state 값을
						 * 사용한 경우
						 */
						...
						{
							/*
							 * state 가 맞지 않음 localstorage 및 cookie 사용이 불가능한 경우엔
							 * nil_login.setState(....); 로 지정 해야 함.
							 */
...
/*
 * 아래 값을 name 으로 사용할 수 있음. age birthday email enc_id gender id nickname
 * profile_image
 */
